...
describe("UsageStatsPlugin hooks", () => {
  ...
  test("tool.execute.after upserts session and stores tool call", async () => {
    const hooks = await mod.UsageStatsPlugin({
    ...
    const session = checkDb
      ...
      .get("sess-2") as { session_id: string; project_id: string } | null;
    const toolCall = checkDb
      .prepare(
        "SELECT session_id, call_id, tool_name, description FROM tool_calls WHERE call_id = ?",
      )
      .get("call-2") as {
      session_id: string;
      call_id: string;
  ...
  test("chat.params stores agent and model/provider for following tool call", async () => {
    ...
    const row = checkDb
      .prepare(
        "SELECT agent, model_id, provider_id, description FROM tool_calls WHERE call_id = ?",
      )
      .get("call-1") as {
      agent: string;
      model_id: string;
  ...
  test("event session.created upserts metadata", async () => {
    ...
    const row = checkDb
      .prepare(
        "SELECT session_id, project_id, parent_id, title, directory FROM sessions WHERE session_id = ?",
      )
      .get("sess-meta") as {
      session_id: string;
      project_id: string;
